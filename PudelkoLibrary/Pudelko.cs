...
namespace PudelkoLibrary {
    public sealed class Pudelko : IFormattable, IEquatable<Pudelko>, IEnumerable<double>, IComparable<Pudelko> {
        public UnitOfMeasure Unit { get; } = UnitOfMeasure.meter;
        ...
        private double ZrobNaMetry(double a) {
            return Math.Floor((a / (double)Unit) * 1000.0) / 1000.0; 
        }

        public Pudelko(double? a = null, double? b = null, double? c = null, UnitOfMeasure unit = UnitOfMeasure.meter) {
            Unit = unit;
            A = (double)(a != null ? ZrobNaMetry((double)a) : ZrobNaMetry(0.1 * (double)unit));
            B = (double)(b != null ? ZrobNaMetry((double)b) : ZrobNaMetry(0.1 * (double)unit));
            C = (double)(c != null ? ZrobNaMetry((double)c) : ZrobNaMetry(0.1 * (double)unit));
